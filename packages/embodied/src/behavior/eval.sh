...
# shared GPU run the whole script under that GPU's lock.
#
# Each episode runs in <out>/<task>_s<seed>/ and ends with a result.json taken from the session's
# `robot_result` entry (success, q_score, the reference `picked` judgement). An episode is valid when the environment produced a result and the planner
...
const configs = new Set(scoredRows
	.map((r) => `${r.model}/${r.thinking}/turns=${r.max_turns}/limit=${r.time_limit}/units=${r.units}${r.units_wrist_view === false ? `/no-wrist:${(r.units_plugins ?? []).join("+")}` : ""}${r.stateless ? "/stateless" : ""}${r.anchor_image ? "/anchor" : ""}${r.vdm ? `/vdm=${r.vdm_model ?? "default"}${r.vdm_wrist ? "+wrist" : ""}` : ""}${r.privileged ? "/privileged" : ""}${r.fallback_model ? `/fallback=${r.fallback_model}:${r.fallback_after}:${r.fallback_retry_primary}` : ""}/grasp=${r.grasping_mode ?? "sticky"}`));
if (configs.size > 1) {
...
const q = scored ? (scoredRows.reduce((a, r) => a + (Number(r.q_score) || 0), 0) / scored).toFixed(3) : "-";
