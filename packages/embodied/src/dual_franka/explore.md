...
2. After a reset, re-read the new state and re-localize. Never reuse a pixel, point or TCP target from an earlier attempt; `back_project` and `segment` refuse steps from before the last reset. A reset that fails or is not confirmed does not start a new attempt; keep motion stopped until a reset completes.
3. Read describe_dual_franka_setup for the registered tools and cameras. Use move_delta, rotate_delta, the gripper tools, recover_joint_posture, back_project, optional segment, and the named vla_right_grasp, vla_handoff and vla_left_place skills. Do not assume LIBERO primitives exist.
