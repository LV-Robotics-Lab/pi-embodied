...
## Workflow
1. When this session started, the operator restored the tabletop and confirmed it, and both arms were reset. `reset` asks the operator to restore the scene again: a human removes or secures held objects, restores the objects and chooses done; the robot posture is then reset and a fresh state step is recorded. Nothing restores the tabletop automatically.
2. After a reset, re-read the new state and re-localize. Never reuse a pixel, point or TCP target from an earlier attempt[tool:back_project|segment]; steps from before the last reset are refused by [tool:back_project]`back_project`[/tool:back_project][tool:back_project][tool:segment] and [/tool:segment][/tool:back_project][tool:segment]`segment`[/tool:segment][/tool:back_project|segment]. A reset that fails or is not confirmed does not start a new attempt; keep motion stopped until a reset completes.
3. Read describe_dual_franka_setup for the registered tools and cameras. Use move_delta, rotate_delta, the gripper tools[tool:recover_joint_posture], recover_joint_posture[/tool:recover_joint_posture][tool:back_project], back_project[/tool:back_project][tool:segment], optional segment[/tool:segment][tool:vla_right_grasp|vla_handoff|vla_left_place], and the named [tool:vla_right_grasp]vla_right_grasp[/tool:vla_right_grasp][tool:vla_handoff][tool:vla_right_grasp], [/tool:vla_right_grasp]vla_handoff[/tool:vla_handoff][tool:vla_left_place][tool:vla_right_grasp|vla_handoff] and [/tool:vla_right_grasp|vla_handoff]vla_left_place[/tool:vla_left_place] skill(s)[/tool:vla_right_grasp|vla_handoff|vla_left_place]. Do not assume LIBERO primitives exist.
