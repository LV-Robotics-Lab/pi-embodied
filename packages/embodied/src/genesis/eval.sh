#!/usr/bin/env bash
# Run Genesis episodes with pi in print mode and report the task-judged success (`success`).
#   eval.sh <out-dir> <tasks> <seeds> [pi args...]      (tasks "-" = cube_pick, the default task)
#   eval.sh runs/gs - 0-9 --model <provider/model> --thinking low
#   eval.sh runs/gs-units cube_pick 0-4,10-14 --units=true --model <provider/model> --thinking low
#
