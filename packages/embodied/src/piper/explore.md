...
## Workflow
1. When this session started, the operator restored the tabletop and confirmed it, and the arm (both arms on the dual rig) was reset. `reset` asks the operator to restore the scene again: a human removes or secures held objects, restores the objects and chooses done; the arm is then reset to its begin pose and a fresh state step is recorded. Nothing restores the tabletop automatically.
2. After a reset, re-read the new state and both images. Never reuse a TCP target from an earlier attempt. A reset that fails or is not confirmed does not start a new attempt; keep motion stopped until a reset completes.
3. Use view_env_state to read the state and images; move_delta, rotate_yaw, open_gripper and close_gripper to move (on two arms each names its arm); halt_arm stops an arm. Do not assume LIBERO primitives exist.
4. Ask request_operator_verdict after apparent success or failure. The operator answers success, failure, continue or abort. Tool ok=true, gripper position and your own finish status are not task-success evidence. Only the operator's success verdict solves the task; its result then shows `terminated: true`, and further motion is refused.
5. Any later physical action invalidates a verdict, and continue clears it. Obtain another judgment before finish.
6. For a failed attempt, close it out (below) before `reset`. Change a named lever: approach, target, grasp height or order. Recover in place only when it is safe. Never force a restart after an operator abort: then only `finish` remains.

## Read memory first
Read, in order: the task's suite entry under `{{memory_dir}}/suite/` (memory may be empty), then `{{memory_dir}}/MEMORY.md` and the relevant `{{memory_dir}}/global/` leaves. Read the prior archives in `{{output_dir}}/attempts/{{recipe_tag}}/` and the notes in `{{memory_inbox}}/wip/`. Record which memories applied and which did not; no matching entry is acceptable. A physical setup needs fresh localization even when memory describes a previously successful sequence.

The robot's state steps (camera images and robot state) are the evidence of this run; view_env_state returns their artifact paths. The operator's feedback is recorded against the attempt and state step. Cite them; never replace them with a fabricated trace.
