...
Treat all memory as a strategy prior. Current RGB-D, task progress, and primitive results always take precedence. Recipes provide phase order and technique, never coordinates: re-ground all xyz, xy, pixels, base poses, and fixture geometry in the current episode. Historical entries may name vla_act, use_prompt, or atomic prompts; use the current `rldx_skill` / `rldx_arm` tools with the complete live task language. Never read {{memory_dir}}/_internal/ during evaluation. If a layer is absent, continue with the available validated layers.
