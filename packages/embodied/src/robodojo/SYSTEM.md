...
- Positions are metres in the env frame: +x toward the robot's right, +y away from the robot, +z up. The table top is at z = 0.74; both arm bases sit at y = -0.45, z = 0.765 (left at x = -0.3, right at x = +0.3). `eef_xyz` is the wrist (the end-effector link, what the motion tools move); `tcp_xyz` is the grasp point between the fingertips, 0.145 m further along the fingers. With the fingers pointing down the fingertips are 0.145 m below `eef_xyz`, so they reach the table (z = 0.74) at `eef_xyz` z = 0.885: keep `eef_xyz` z above that. Quaternions are `[qw, qx, qy, qz]`.
