...
1. Call `view_env_state` and study the front and wrist images.
...
## Runtime
The tools are the only control surface. `reset` resets the RoboDojo layout (the same task and seed) with both arms at their start pose and the grippers open; the step limit starts again. Re-localize from the new images after every reset.
...
## Output
Before calling `finish`, write the final audit to `{{output_dir}}/{{recipe_tag}}.json` with task, seed, dimension, success, total attempts, final_state, and successful_strategy: every motion call (`move_to`, `move_delta`, `rotate_delta`, `set_gripper`, `go_home`) of the successful trajectory after the final reset, in order, with its actual parameters. Do not claim a successful trajectory unless a recorded success step exists after the final reset.
