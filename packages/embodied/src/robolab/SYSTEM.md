...
# Mechanics
- The gripper points straight down and keeps its heading[tool:rotate_delta] unless `rotate_delta` turns it about the vertical by `yaw` radians (+ counter-clockwise seen from above, at most 0.3 rad per call; `yaw_deg` is the heading from the start, and the wrist view turns with the gripper)[/tool:rotate_delta].[tool:move_delta] `move_delta` takes a base-frame `[dx, dy, dz]` in metres: +x away from the robot base, +y toward the robot's left, +z up.[/tool:move_delta] `eef_pos` is the gripper's hand body; the fingertips reach about 10 cm below it.
