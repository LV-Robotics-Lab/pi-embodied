...
# differencing (--vdm, --vdm-model, --vdm-wrist), the task phrasing (--instruction-type) and --subtask,
...
for ((i = 0; i < ${#args[@]}; i++)); do
	...
	--anchor-image=*)
		...
		exit 2
		;;
	# RoboLab's task phrasing (default, vague, specific) changes the task: never one summary across phrasings.
	--instruction-type) instruction=${args[i + 1]:-} ;;
	--instruction-type=*) instruction=${args[i]#*=} ;;
	# --subtask (RoboLab's partial-credit score) is a boolean like --stateless.
	--subtask) case ${args[i + 1]:-} in "" | -* | @* | true) subtask=true ;; *)
		echo "--subtask takes no value: pi would turn it on and swallow '${args[i + 1]}'" >&2 && exit 2 ;;
	esac ;;
	--subtask=true) subtask=true ;;
	--subtask=*)
		echo "${args[i]}: pi ignores a boolean flag's value and would turn subtask tracking on; omit --subtask for a run without it" >&2
		exit 2
		;;
	esac
done
...
mkdir -p "$out"
config=("$model" "$thinking" "$turns" "$limit" "$units" "$stateless" "$anchor" "$vdm" "$vdm_model" "$vdm_wrist" "$privileged" "$instruction" "$subtask" "$fallback_model" "$fallback_after" "$fallback_retry")

record() { # <dir> <exit code>: write result.json from the episode's session
	node --input-type=module -e '
import { readdirSync, readFileSync, writeFileSync } from "node:fs";
const [dir, code, model, thinking, turns, limit, units, stateless, anchor, vdm, vdmModel, vdmWrist, privileged, instruction, subtask, fallbackModel, fallbackAfter, fallbackRetry] = process.argv.slice(1);
const results = [];
...
valid() { # <dir>: 0 = a valid result of this configuration, 2 = a valid result of another one, 1 = none
	node -e '
const [path, model, thinking, turns, limit, units, stateless, anchor, vdm, vdmModel, vdmWrist, privileged, instruction, subtask, fallbackModel, fallbackAfter, fallbackRetry] = process.argv.slice(1);
...
for task in ${tasks//,/ }; do
	for seed in $(expand "$seeds"); do
		...
		case $? in
		0) continue ;;
		2) echo "$dir holds a result of another model, thinking level, --max-turns, --time-limit, units mode, vdm, --instruction-type or --subtask (or an older result without them), fallback, --privileged or --anchor-image; use another out dir" >&2 && exit 1 ;;
		esac
...
const configs = new Set(rows.filter((r) => r.status === "success" || r.status === "failure")
	.map((r) => `${r.model}/${r.thinking}/turns=${r.max_turns}/limit=${r.time_limit}/units=${r.units}${r.stateless ? "/stateless" : ""}${r.anchor_image ? "/anchor" : ""}${r.vdm ? `/vdm=${r.vdm_model ?? "default"}${r.vdm_wrist ? "+wrist" : ""}` : ""}${r.privileged ? "/privileged" : ""}${r.fallback_model ? `/fallback=${r.fallback_model}:${r.fallback_after}:${r.fallback_retry_primary}` : ""}/${r.instruction_type}${r.subtask ? "/subtask" : ""}`));
if (configs.size > 1) {
