#!/usr/bin/env bash
# Run robosuite episodes with pi in print mode and report robosuite-judged success (`success`).
#   eval.sh <out-dir> <tasks> <seeds> [pi args...]      (tasks "-" = all seven CaP-X tasks)
#   eval.sh runs/rs - 0-9 --model <provider/model> --thinking low
#   eval.sh runs/rs Lift,Stack,Restack 0-9 --model <provider/model> --thinking low
#   eval.sh runs/rs-units Lift 0-4,10-14 --units=true --model <provider/model> --thinking low
#
...
# --max-move (the per-call travel cap, m; the robot's default 0.3, src/robosuite/index.ts MAX_MOVE_M) is recorded too.
set -uo pipefail
out=$1 tasks=$2 seeds=$3
[ "$tasks" = - ] && tasks=Lift,Stack,Restack,Wipe,NutAssemblySquare,TwoArmLift,TwoArmHandover
shift 3
...
for ((i = 0; i < ${#args[@]}; i++)); do
	...
	--max-move) max_move=${args[i + 1]:-} ;;
	--max-move=*) max_move=${args[i]#*=} ;;
...
mkdir -p "$out"
config=("$model" "$thinking" "$turns" "$limit" "$units" "$stateless" "$privileged" "$anchor" "$vdm" "$vdm_model" "$vdm_wrist" "$fallback_model" "$fallback_after" "$fallback_retry" "$max_move")

record() { # <dir> <exit code>: write result.json from the episode's session
	node --input-type=module -e '
import { readdirSync, readFileSync, writeFileSync } from "node:fs";
const [dir, code, model, thinking, turns, limit, units, stateless, privileged, anchor, vdm, vdmModel, vdmWrist, fallbackModel, fallbackAfter, fallbackRetry, maxMove] = process.argv.slice(1);
const results = [];
...
valid() { # <dir>: 0 = a valid result of this configuration, 2 = a valid result of another one, 1 = none
	node -e '
const [path, model, thinking, turns, limit, units, stateless, privileged, anchor, vdm, vdmModel, vdmWrist, fallbackModel, fallbackAfter, fallbackRetry, maxMove] = process.argv.slice(1);
...
const same = r.model === (model || null) && r.thinking === (thinking || null) && r.max_turns === Number(turns)
	...
	// Results written before --max-move was recorded ran with the default.
	&& (r.max_move ?? 0.3) === Number(maxMove);
...
for task in ${tasks//,/ }; do
	for seed in $(expand "$seeds"); do
		...
		case $? in
		0) continue ;;
		2) echo "$dir holds a result of another model, thinking level, --max-turns, --time-limit, units mode, vdm, fallback, --max-move, --privileged or --anchor-image; use another out dir" >&2 && exit 1 ;;
		esac
...
const configs = new Set(rows.filter((r) => r.status === "success" || r.status === "failure")
	.map((r) => `${r.model}/${r.thinking}/turns=${r.max_turns}/limit=${r.time_limit}/units=${r.units}${r.stateless ? "/stateless" : ""}${r.anchor_image ? "/anchor" : ""}${r.vdm ? `/vdm=${r.vdm_model ?? "default"}${r.vdm_wrist ? "+wrist" : ""}` : ""}${r.privileged ? "/privileged" : ""}${r.fallback_model ? `/fallback=${r.fallback_model}:${r.fallback_after}:${r.fallback_retry_primary}` : ""}/max_move=${r.max_move ?? 0.3}`));
if (configs.size > 1) {
