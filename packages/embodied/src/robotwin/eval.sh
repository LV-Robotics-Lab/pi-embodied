...
# records the model, thinking level and --max-turns, and the summary covers only the requested cells and
...
const configs = new Set(rows.filter((r) => r.status === "success" || r.status === "failure").map((r) => `${r.model}/${r.thinking}/turns=${r.max_turns}`));
if (configs.size > 1) {
