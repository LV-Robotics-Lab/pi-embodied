...
# --stateless), visual differencing (--vdm, --vdm-model, --vdm-wrist) and the env server's planner
...
planner=curobo_lbfgs=torch,deterministic=true # = contract.py ROBOTWIN_PLANNER (test/eval-scripts.test.ts)
args=("$@")
for ((i = 0; i < ${#args[@]}; i++)); do
	case ${args[i]} in
...
const configs = new Set(rows.filter((r) => r.status === "success" || r.status === "failure")
	.map((r) => `${r.model}/${r.thinking}/turns=${r.max_turns}/limit=${r.time_limit}/units=${r.units}${r.stateless ? "/stateless" : ""}${r.anchor_image ? "/anchor" : ""}${r.vdm ? `/vdm=${r.vdm_model ?? "default"}${r.vdm_wrist ? "+wrist" : ""}` : ""}${r.privileged ? "/privileged" : ""}${r.fallback_model ? `/fallback=${r.fallback_model}:${r.fallback_after}:${r.fallback_retry_primary}` : ""}/${r.planner}`));
if (configs.size > 1) {
