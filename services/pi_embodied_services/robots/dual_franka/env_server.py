...
#
# Modified by pi-embodied: import paths rewritten; healthz service name; the
# worker's servo/chunk loops and joint-posture recovery poll the stop flag
# between steps (results carry ``cancelled``).
...
from pi_embodied_services.robots.dual_franka.runtime_config import load_runtime_config
from pi_embodied_services.robots.franka.env_server import (
    FrankaEnvFacade,
    _StopPoller,
    main,
)
...
def _create_worker_class():
    ...
    from pi_embodied_services.robots.dual_franka.perception import (
    ...
    class DualFrankaEnvWorker(Worker):
        ...
        def recover_joint_posture(
            ...
            stopped = self._stop.requested()
            return {
                **({"cancelled": True} if stopped else {}),
