...
#
# Modified by pi-embodied: copied from core/piper/kinematics.py unchanged except
# for this header and formatting.
...
class PiperKinematics:
    ...
    def ik(
        ...
        for _ in range(max_iters):
            ...
            if (
                float(np.linalg.norm(e_pos)) < pos_tol_m
                and float(np.linalg.norm(e_ori)) < ori_tol_rad
            ):
        ...
        if (
            ...
            and float(np.linalg.norm(R.from_matrix(target_rot @ rot.T).as_rotvec()))
            < ori_tol_rad
        ):
            return q if self._near_seed(q, q_seed, max_seed_dist_rad) else None
        if _retry:
            ...
            return self.ik(
                ...
                max_iters=max_iters,
                damping=0.01,
                max_seed_dist_rad=max_seed_dist_rad,
                _retry=False,
            )
        return None
