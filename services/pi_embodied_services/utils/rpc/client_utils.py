...
#
# Modified by pi-embodied: import paths rewritten; the socket protocol is removed
# (HTTP only) and ``wait_for_ready`` takes any object with ``poll()``/``name``.
...
if TYPE_CHECKING:
    from pi_embodied_services.utils.rpc.rpc_client import RpcClient
...
def make_rpc_client(endpoint: str, *, enable_sessions: bool = False) -> "RpcClient":
    """Build an HTTP client for ``endpoint``."""
