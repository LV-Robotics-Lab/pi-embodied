...
#
# Modified by pi-embodied: import paths rewritten; lock-free methods (healthz,
# stop/cancel, shutdown) are answered on the transport thread and business calls
# carry their arrival stop generation into the locked ``RpcFacade._run_call``;
...
:meth:`pi_embodied_services.utils.rpc.rpc_facade.RpcFacade.serve` to run the transport
...
needs this; otherwise inherit :class:`pi_embodied_services.utils.rpc.rpc_facade.RpcFacade`
...
from pi_embodied_services.utils.rpc.rpc_facade import LOCK_FREE_METHODS
...
class MainThreadServeMixin:
    ...
    def _dispatch_main_thread(
        ...
        methods (``healthz`` / ``stop`` / ``cancel`` / ``shutdown``) are
        answered through :meth:`RpcFacade._builtin_dispatch` without waiting
